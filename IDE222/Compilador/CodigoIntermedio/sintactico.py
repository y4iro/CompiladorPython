...
def listasentencias():
    ...
    if (token.contenido=="if" or token.contenido=="while" or
        ...
        #if t!=None:
        q=t
        while (token.contenido=="if" or token.contenido=="while" or token.contenido=="repeat" or token.contenido=="cin" or token.contenido=="cout"
               or token.contenido=="{" or token.tipo=="Identificador"):
            p=sentencia()
            t.hermano=p
            t=p
    #checkinput(slistasentencias,plistasentencias)"""
    t=None
    while(token.contenido!="else" and token.contenido!="{" and token.contenido!="until" and token.contenido!="then" and token.contenido!="}"):
        q=sentencia()
        if(q!=None):
            if(t==None):
                t=p=q
...
def imprime(arbol,tabs):
    if(arbol != None):
        ...
        print( texto + " " + arbol.contenido +"__"+arbol.tipo)
        outfile = open('MuestraSintaxis.txt', 'a')
        outfile.write(texto + " " + arbol.contenido + "--"+arbol.categoria+"--"+arbol.tipo+ "\n")
        outfile.close()
...
def archivos_errores():
    arbol=sintaxis()
    outfile = open('MuestraSintaxis.txt', 'w')
